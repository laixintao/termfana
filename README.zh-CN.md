...
搜索时可直接用 ↑/↓ 选择结果、PgUp/PgDn 翻页，Enter 立即确认当前选中项。Esc 结束搜索输入并保留筛选和选中项；←/→ 用于移动文字光标。
...
make check           # race tests、go vet 和版本一致性校验
...
测试涵盖格式解析、HTTP 鉴权/超时、Counter 重置、Histogram 区间计算、内存淘汰、CLI JSON、终端尺寸和键盘流程。`make smoke` 会启动临时 localhost 服务，验证四面板、标签筛选、断采恢复、保存/恢复、窗口缩放，以及正常退出、SIGINT、SIGTERM 后的终端恢复。脚本可仅使用 Python 标准库运行；安装 `pyte` 后还会校验实际渲染画面，CI 使用这种方式。
